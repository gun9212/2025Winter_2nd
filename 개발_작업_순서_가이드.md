# 개발 작업 순서 가이드

백엔드와 프론트엔드를 구분하지 않고, 기능별로 완성하는 순서로 정리한 상세 작업 가이드입니다.

---

## 📋 전체 작업 순서

```
Phase 1: 위치 업데이트 API 완성 (최우선)
  ↓
Phase 2: 프로필 관리 시스템 완성
  ↓
Phase 3: 이상형 프로필 시스템 완성
  ↓
Phase 4: 매칭 알고리즘 & 시스템 완성
  ↓
Phase 5: 서비스 설정 기능 완성
  ↓
Phase 6: 알림 시스템 완성
  ↓
Phase 7: 통합 테스트 & 최종 점검
```

---

## Phase 1: 위치 업데이트 API 완성 ⭐⭐⭐⭐⭐

**목표**: GPS 위치 정보를 DB에 저장하는 기능 완성

**예상 소요 시간**: 1-2시간

### 1-1. 백엔드: Serializer 생성

**파일**: `backend/apps/users/serializers.py`

**작업 내용**:
```python
# 기존 파일에 추가
from rest_framework import serializers
from .models import UserLocation

class UserLocationSerializer(serializers.ModelSerializer):
    class Meta:
        model = UserLocation
        fields = ['latitude', 'longitude']
    
    def validate_latitude(self, value):
        if not (-90 <= value <= 90):
            raise serializers.ValidationError("위도는 -90~90 사이여야 합니다.")
        return value
    
    def validate_longitude(self, value):
        if not (-180 <= value <= 180):
            raise serializers.ValidationError("경도는 -180~180 사이여야 합니다.")
        return value
```

**체크리스트**:
- [ ] `UserLocationSerializer` 클래스 생성
- [ ] 위도/경도 범위 검증 추가
- [ ] 저장 테스트

---

### 1-2. 백엔드: View 구현

**파일**: `backend/apps/users/views.py`

**작업 내용**:
```python
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import IsAuthenticated
from rest_framework.response import Response
from rest_framework import status
from .models import UserLocation
from .serializers import UserLocationSerializer

@api_view(['POST'])
@permission_classes([IsAuthenticated])
def update_location(request):
    serializer = UserLocationSerializer(data=request.data)
    if serializer.is_valid():
        # upsert (있으면 업데이트, 없으면 생성)
        location, created = UserLocation.objects.update_or_create(
            user=request.user.profile,  # AuthUser -> User 프로필
            defaults={
                'latitude': serializer.validated_data['latitude'],
                'longitude': serializer.validated_data['longitude'],
            }
        )
        return Response({
            'success': True,
            'message': '위치가 업데이트되었습니다.',
            'data': UserLocationSerializer(location).data
        }, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
```

**체크리스트**:
- [ ] `update_location` 함수 구현
- [ ] JWT 인증 확인
- [ ] upsert 로직 구현
- [ ] 에러 처리 추가

---

### 1-3. 백엔드: URL 라우팅

**파일**: `backend/apps/users/urls.py`

**작업 내용**:
```python
from django.urls import path
from . import views

app_name = 'users'

urlpatterns = [
    path('location/update/', views.update_location, name='update_location'),
]
```

**체크리스트**:
- [ ] URL 패턴 추가
- [ ] 경로 확인: `/api/users/location/update/`

---

### 1-4. 백엔드 테스트

**작업 내용**:
```bash
# Django 서버 실행
cd backend
source venv/bin/activate
python manage.py runserver

# 다른 터미널에서 테스트 (Postman 또는 curl)
curl -X POST http://localhost:8000/api/users/location/update/ \
  -H "Authorization: Bearer <access_token>" \
  -H "Content-Type: application/json" \
  -d '{"latitude": 37.5665, "longitude": 126.9780}'
```

**체크리스트**:
- [ ] 로그인 후 토큰 발급 확인
- [ ] 위치 업데이트 API 테스트 성공
- [ ] DB에 위치 정보 저장 확인
- [ ] 에러 케이스 테스트 (잘못된 위도/경도 등)

---

### 1-5. 프론트엔드 확인 및 수정

**파일**: `Frontend/IdealMatchApp/src/services/api/apiClient.js`

**확인 사항**:
- [ ] `updateLocation` 함수가 올바른 엔드포인트 호출하는지 확인
- [ ] 에러 처리가 올바른지 확인
- [ ] 토큰이 자동으로 첨부되는지 확인

**필요시 수정**:
```javascript
// 현재 코드 확인 후 경로가 다르면 수정
async updateLocation(latitude, longitude) {
  const response = await this.request('/users/location/update/', {
    method: 'POST',
    body: JSON.stringify({
      latitude,
      longitude,
    }),
  });
  // ...
}
```

**체크리스트**:
- [ ] API 엔드포인트 경로 확인
- [ ] 실제 앱에서 위치 전송 테스트
- [ ] 네트워크 에러 처리 확인

---

### Phase 1 완료 기준

- [ ] 백엔드 API가 정상 작동
- [ ] 프론트엔드에서 위치 정보가 DB에 저장됨
- [ ] 에러 처리가 정상 작동

---

## Phase 2: 프로필 관리 시스템 완성 ⭐⭐⭐⭐

**목표**: 사용자 프로필 조회, 생성, 수정 기능 완성

**예상 소요 시간**: 2-3시간

### 2-1. 백엔드: UserSerializer 생성

**파일**: `backend/apps/users/serializers.py`

**작업 내용**:
```python
from .models import User

class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = User
        fields = ['age', 'gender', 'height', 'mbti', 'personality', 'interests']
        read_only_fields = ['created_at', 'updated_at']
    
    def validate_personality(self, value):
        if not value or len(value) == 0:
            raise serializers.ValidationError("성격 유형을 최소 1개 이상 선택해주세요.")
        return value
    
    def validate_interests(self, value):
        if not value or len(value) == 0:
            raise serializers.ValidationError("관심사를 최소 1개 이상 선택해주세요.")
        return value
```

**체크리스트**:
- [ ] `UserSerializer` 클래스 생성
- [ ] 필수 필드 검증 추가
- [ ] 성격/관심사 최소 1개 검증

---

### 2-2. 백엔드: 프로필 View 구현

**파일**: `backend/apps/users/views.py`

**작업 내용**:
```python
from .models import User
from .serializers import UserSerializer

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_profile(request):
    try:
        profile = request.user.profile
        serializer = UserSerializer(profile)
        return Response({
            'success': True,
            'data': serializer.data
        }, status=status.HTTP_200_OK)
    except User.DoesNotExist:
        return Response({
            'success': False,
            'message': '프로필이 없습니다.'
        }, status=status.HTTP_404_NOT_FOUND)

@api_view(['POST', 'PUT'])
@permission_classes([IsAuthenticated])
def update_profile(request):
    try:
        profile = request.user.profile
        serializer = UserSerializer(profile, data=request.data, partial=request.method == 'PUT')
    except User.DoesNotExist:
        # 프로필이 없으면 생성
        serializer = UserSerializer(data=request.data)
    
    if serializer.is_valid():
        serializer.save(user=request.user.profile.user if hasattr(request.user, 'profile') else request.user)
        return Response({
            'success': True,
            'message': '프로필이 저장되었습니다.',
            'data': serializer.data
        }, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def check_profile_completeness(request):
    try:
        profile = request.user.profile
        ideal_type = profile.ideal_type_profile
        
        # 프로필 완성도 체크
        profile_fields = ['age', 'gender', 'height', 'mbti', 'personality', 'interests']
        profile_complete = all(getattr(profile, field, None) for field in profile_fields)
        profile_complete = profile_complete and len(profile.personality) > 0 and len(profile.interests) > 0
        
        # 이상형 프로필 완성도 체크
        ideal_type_complete = False
        if ideal_type:
            ideal_fields = ['height_min', 'height_max', 'age_min', 'age_max', 
                          'preferred_mbti', 'preferred_personality', 'preferred_interests']
            ideal_type_complete = all(getattr(ideal_type, field, None) for field in ideal_fields)
            ideal_type_complete = ideal_type_complete and \
                len(ideal_type.preferred_mbti) > 0 and \
                len(ideal_type.preferred_personality) > 0 and \
                len(ideal_type.preferred_interests) > 0
        
        return Response({
            'success': True,
            'profile_complete': profile_complete,
            'ideal_type_complete': ideal_type_complete,
            'all_complete': profile_complete and ideal_type_complete
        }, status=status.HTTP_200_OK)
    except User.DoesNotExist:
        return Response({
            'success': False,
            'profile_complete': False,
            'ideal_type_complete': False,
            'all_complete': False
        }, status=status.HTTP_200_OK)
```

**체크리스트**:
- [ ] `get_profile` 함수 구현
- [ ] `update_profile` 함수 구현 (생성/수정)
- [ ] `check_profile_completeness` 함수 구현
- [ ] 에러 처리 추가

---

### 2-3. 백엔드: URL 추가

**파일**: `backend/apps/users/urls.py`

**작업 내용**:
```python
urlpatterns = [
    path('location/update/', views.update_location, name='update_location'),
    path('profile/', views.get_profile, name='get_profile'),
    path('profile/', views.update_profile, name='update_profile'),  # POST/PUT
    path('profile/completeness/', views.check_profile_completeness, name='check_profile_completeness'),
]
```

**체크리스트**:
- [ ] URL 패턴 추가
- [ ] 경로 확인

---

### 2-4. 프론트엔드: API 클라이언트 함수 추가

**파일**: `Frontend/IdealMatchApp/src/services/api/apiClient.js`

**작업 내용**:
```javascript
/**
 * 프로필 조회
 * @returns {Promise<Object>} 프로필 정보
 */
async getProfile() {
  try {
    const response = await this.request('/users/profile/', {
      method: 'GET',
    });
    return {
      success: true,
      data: response.data,
    };
  } catch (error) {
    console.error('❌ 프로필 조회 실패:', error);
    return {
      success: false,
      error: error.message,
    };
  }
}

/**
 * 프로필 생성/수정
 * @param {Object} profileData - 프로필 데이터
 * @returns {Promise<Object>} 저장 결과
 */
async updateProfile(profileData) {
  try {
    const response = await this.request('/users/profile/', {
      method: 'POST',  // 또는 PUT (기존 프로필이 있으면 PUT)
      body: JSON.stringify(profileData),
    });
    return {
      success: true,
      data: response.data,
    };
  } catch (error) {
    console.error('❌ 프로필 저장 실패:', error);
    throw error;
  }
}

/**
 * 프로필 완성도 확인
 * @returns {Promise<Object>} 완성도 정보
 */
async checkProfileCompleteness() {
  try {
    const response = await this.request('/users/profile/completeness/', {
      method: 'GET',
    });
    return {
      success: true,
      ...response,
    };
  } catch (error) {
    console.error('❌ 프로필 완성도 확인 실패:', error);
    return {
      success: false,
      error: error.message,
    };
  }
}
```

**체크리스트**:
- [ ] `getProfile` 함수 추가
- [ ] `updateProfile` 함수 추가
- [ ] `checkProfileCompleteness` 함수 추가

---

### 2-5. 프론트엔드: AuthContext 연동

**파일**: `Frontend/IdealMatchApp/src/context/AuthContext.js`

**작업 내용**:
```javascript
import { apiClient } from '../services/api';

// 기존 updateProfile 함수 수정
const updateProfile = async (profile) => {
  try {
    if (!currentUser?.userId) {
      throw new Error('로그인된 사용자가 없습니다.');
    }
    
    // 실제 API 호출
    const result = await apiClient.updateProfile(profile);
    
    if (!result.success) {
      throw new Error(result.error || '프로필 업데이트에 실패했습니다.');
    }
    
    // 로컬 저장소에도 저장 (선택사항)
    await StorageService.saveUserProfile(profile, currentUser.userId);
    setUserProfile(profile);
    
    console.log('✅ 프로필 업데이트 완료');
  } catch (error) {
    console.error('Update profile error:', error);
    throw error;
  }
};

// 프로필 조회 함수 추가
const loadProfile = async () => {
  try {
    if (!currentUser?.userId) {
      return;
    }
    
    const result = await apiClient.getProfile();
    if (result.success && result.data) {
      setUserProfile(result.data);
      await StorageService.saveUserProfile(result.data, currentUser.userId);
    }
  } catch (error) {
    console.error('Load profile error:', error);
  }
};
```

**체크리스트**:
- [ ] `updateProfile` 함수 실제 API 호출로 변경
- [ ] `loadProfile` 함수 추가
- [ ] 앱 시작 시 프로필 자동 로드

---

### 2-6. 프론트엔드: 프로필 화면 연동

**파일**: `Frontend/IdealMatchApp/src/screens/Profile/ProfileInputScreen.js`

**작업 내용**:
- 기존 Mock API 호출 부분을 `apiClient.updateProfile()`로 변경
- 에러 메시지 표시 개선
- 로딩 상태 표시

**체크리스트**:
- [ ] API 호출 로직 수정
- [ ] 에러 처리 개선
- [ ] 실제 앱에서 테스트

---

### Phase 2 완료 기준

- [ ] 프로필 조회 API 정상 작동
- [ ] 프로필 생성/수정 API 정상 작동
- [ ] 프로필 완성도 확인 API 정상 작동
- [ ] 프론트엔드에서 프로필 저장/조회 정상 작동

---

## Phase 3: 이상형 프로필 시스템 완성 ⭐⭐⭐⭐

**목표**: 이상형 프로필 조회, 생성, 수정 기능 완성

**예상 소요 시간**: 2-3시간

### 3-1. 백엔드: IdealTypeProfileSerializer 생성

**파일**: `backend/apps/users/serializers.py`

**작업 내용**:
```python
from .models import IdealTypeProfile

class IdealTypeProfileSerializer(serializers.ModelSerializer):
    class Meta:
        model = IdealTypeProfile
        fields = ['height_min', 'height_max', 'age_min', 'age_max', 
                 'preferred_mbti', 'preferred_personality', 'preferred_interests', 
                 'match_threshold']
        read_only_fields = ['created_at', 'updated_at']
    
    def validate(self, data):
        # 키 범위 검증
        if data.get('height_min') and data.get('height_max'):
            if data['height_min'] > data['height_max']:
                raise serializers.ValidationError({
                    'height': '최소 키는 최대 키보다 작거나 같아야 합니다.'
                })
        
        # 나이 범위 검증
        if data.get('age_min') and data.get('age_max'):
            if data['age_min'] > data['age_max']:
                raise serializers.ValidationError({
                    'age': '최소 나이는 최대 나이보다 작거나 같아야 합니다.'
                })
        
        # 선호 MBTI 검증
        if data.get('preferred_mbti') and len(data['preferred_mbti']) == 0:
            raise serializers.ValidationError({
                'preferred_mbti': 'MBTI를 최소 1개 이상 선택해주세요.'
            })
        
        # 선호 성격 검증
        if data.get('preferred_personality') and len(data['preferred_personality']) == 0:
            raise serializers.ValidationError({
                'preferred_personality': '성격 유형을 최소 1개 이상 선택해주세요.'
            })
        
        # 선호 관심사 검증
        if data.get('preferred_interests') and len(data['preferred_interests']) == 0:
            raise serializers.ValidationError({
                'preferred_interests': '관심사를 최소 1개 이상 선택해주세요.'
            })
        
        return data
```

**체크리스트**:
- [ ] `IdealTypeProfileSerializer` 클래스 생성
- [ ] 범위 검증 로직 추가
- [ ] 필수 필드 검증 추가

---

### 3-2. 백엔드: 이상형 프로필 View 구현

**파일**: `backend/apps/users/views.py`

**작업 내용**:
```python
from .models import IdealTypeProfile
from .serializers import IdealTypeProfileSerializer

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_ideal_type(request):
    try:
        ideal_type = request.user.profile.ideal_type_profile
        serializer = IdealTypeProfileSerializer(ideal_type)
        return Response({
            'success': True,
            'data': serializer.data
        }, status=status.HTTP_200_OK)
    except (User.DoesNotExist, IdealTypeProfile.DoesNotExist):
        return Response({
            'success': False,
            'message': '이상형 프로필이 없습니다.'
        }, status=status.HTTP_404_NOT_FOUND)

@api_view(['POST', 'PUT'])
@permission_classes([IsAuthenticated])
def update_ideal_type(request):
    try:
        profile = request.user.profile
        ideal_type = profile.ideal_type_profile
        serializer = IdealTypeProfileSerializer(ideal_type, data=request.data, partial=request.method == 'PUT')
    except (User.DoesNotExist, IdealTypeProfile.DoesNotExist):
        # 이상형 프로필이 없으면 생성
        serializer = IdealTypeProfileSerializer(data=request.data)
    
    if serializer.is_valid():
        if hasattr(request.user, 'profile'):
            serializer.save(user=request.user.profile)
        else:
            # User 프로필이 없으면 먼저 생성해야 함
            return Response({
                'success': False,
                'message': '먼저 프로필을 생성해주세요.'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        return Response({
            'success': True,
            'message': '이상형 프로필이 저장되었습니다.',
            'data': serializer.data
        }, status=status.HTTP_200_OK)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
```

**체크리스트**:
- [ ] `get_ideal_type` 함수 구현
- [ ] `update_ideal_type` 함수 구현
- [ ] 에러 처리 추가

---

### 3-3. 백엔드: URL 추가

**파일**: `backend/apps/users/urls.py`

**작업 내용**:
```python
urlpatterns = [
    # ... 기존 URL들
    path('ideal-type/', views.get_ideal_type, name='get_ideal_type'),
    path('ideal-type/', views.update_ideal_type, name='update_ideal_type'),  # POST/PUT
]
```

**체크리스트**:
- [ ] URL 패턴 추가

---

### 3-4. 프론트엔드: API 클라이언트 함수 추가

**파일**: `Frontend/IdealMatchApp/src/services/api/apiClient.js`

**작업 내용**:
```javascript
/**
 * 이상형 프로필 조회
 * @returns {Promise<Object>} 이상형 프로필 정보
 */
async getIdealType() {
  try {
    const response = await this.request('/users/ideal-type/', {
      method: 'GET',
    });
    return {
      success: true,
      data: response.data,
    };
  } catch (error) {
    console.error('❌ 이상형 프로필 조회 실패:', error);
    return {
      success: false,
      error: error.message,
    };
  }
}

/**
 * 이상형 프로필 생성/수정
 * @param {Object} idealTypeData - 이상형 프로필 데이터
 * @returns {Promise<Object>} 저장 결과
 */
async updateIdealType(idealTypeData) {
  try {
    const response = await this.request('/users/ideal-type/', {
      method: 'POST',
      body: JSON.stringify(idealTypeData),
    });
    return {
      success: true,
      data: response.data,
    };
  } catch (error) {
    console.error('❌ 이상형 프로필 저장 실패:', error);
    throw error;
  }
}
```

**체크리스트**:
- [ ] `getIdealType` 함수 추가
- [ ] `updateIdealType` 함수 추가

---

### 3-5. 프론트엔드: AuthContext 연동

**파일**: `Frontend/IdealMatchApp/src/context/AuthContext.js`

**작업 내용**:
- `updateIdealType` 함수를 실제 API 호출로 변경
- `loadIdealType` 함수 추가

**체크리스트**:
- [ ] 실제 API 호출로 변경
- [ ] 앱 시작 시 이상형 프로필 자동 로드

---

### 3-6. 프론트엔드: 이상형 프로필 화면 연동

**파일**: `Frontend/IdealMatchApp/src/screens/IdealType/IdealTypeInputScreen.js`

**작업 내용**:
- Mock API 호출 부분을 `apiClient.updateIdealType()`로 변경

**체크리스트**:
- [ ] API 호출 로직 수정
- [ ] 실제 앱에서 테스트

---

### Phase 3 완료 기준

- [ ] 이상형 프로필 조회/생성/수정 API 정상 작동
- [ ] 프론트엔드에서 이상형 프로필 저장/조회 정상 작동

---

## Phase 4: 매칭 알고리즘 & 시스템 완성 ⭐⭐⭐⭐⭐

**목표**: 매칭 알고리즘 구현 및 매칭 체크 기능 완성

**예상 소요 시간**: 4-6시간 (가장 복잡)

### 4-1. 백엔드: 거리 계산 함수 구현

**파일**: `backend/apps/matching/utils.py` (새 파일 생성)

**작업 내용**:
```python
from math import radians, sin, cos, sqrt, atan2

def calculate_distance(lat1, lon1, lat2, lon2):
    """
    하버사인 공식으로 두 지점 간 거리 계산 (km 단위)
    """
    R = 6371  # 지구 반지름 (km)
    
    lat1_rad = radians(float(lat1))
    lon1_rad = radians(float(lon1))
    lat2_rad = radians(float(lat2))
    lon2_rad = radians(float(lon2))
    
    dlat = lat2_rad - lat1_rad
    dlon = lon2_rad - lon1_rad
    
    a = sin(dlat/2)**2 + cos(lat1_rad) * cos(lat2_rad) * sin(dlon/2)**2
    c = 2 * atan2(sqrt(a), sqrt(1-a))
    
    distance = R * c
    return distance

def get_users_within_radius(latitude, longitude, radius_km):
    """
    반경 내 사용자 조회
    """
    from apps.users.models import User, UserLocation
    
    # 모든 활성 사용자 위치 조회
    locations = UserLocation.objects.select_related('user').all()
    
    nearby_users = []
    for location in locations:
        distance = calculate_distance(
            latitude, longitude,
            location.latitude, location.longitude
        )
        if distance <= radius_km:
            nearby_users.append({
                'user': location.user,
                'distance': distance,
                'location': location
            })
    
    return nearby_users
```

**체크리스트**:
- [ ] `calculate_distance` 함수 구현
- [ ] `get_users_within_radius` 함수 구현
- [ ] 테스트 코드 작성 (선택사항)

---

### 4-2. 백엔드: 매칭 알고리즘 구현

**파일**: `backend/apps/matching/utils.py`

**작업 내용**:
```python
from apps.users.models import User, IdealTypeProfile

def check_match_criteria(ideal_type, target_user, match_threshold=3):
    """
    이상형 조건과 대상 사용자 매칭 체크
    반환: (매칭 점수, 매칭된 조건 리스트)
    """
    score = 0
    matched_criteria = []
    
    # 조건 1: 키 범위 체크
    if ideal_type.height_min <= target_user.height <= ideal_type.height_max:
        score += 1
        matched_criteria.append('height')
    
    # 조건 2: 나이 범위 체크
    if ideal_type.age_min <= target_user.age <= ideal_type.age_max:
        score += 1
        matched_criteria.append('age')
    
    # 조건 3: MBTI 체크
    if target_user.mbti in ideal_type.preferred_mbti:
        score += 1
        matched_criteria.append('mbti')
    
    # 조건 4: 성격 교집합 체크
    personality_match = set(target_user.personality) & set(ideal_type.preferred_personality)
    if len(personality_match) > 0:
        score += 1
        matched_criteria.append('personality')
    
    # 조건 5: 관심사 교집합 체크
    interest_match = set(target_user.interests) & set(ideal_type.preferred_interests)
    if len(interest_match) > 0:
        score += 1
        matched_criteria.append('interests')
    
    return score, matched_criteria

def find_matches(user, latitude, longitude, radius_km=0.05):
    """
    사용자에 대한 매칭 후보 찾기
    """
    from apps.matching.utils import get_users_within_radius, check_match_criteria
    
    # 프로필과 이상형 프로필 확인
    try:
        profile = user
        ideal_type = profile.ideal_type_profile
    except (AttributeError, IdealTypeProfile.DoesNotExist):
        return []
    
    # 매칭 동의 및 서비스 활성화 확인
    if not profile.matching_consent or not profile.service_active:
        return []
    
    # 반경 내 사용자 조회
    nearby_users_data = get_users_within_radius(latitude, longitude, radius_km)
    
    matches = []
    for user_data in nearby_users_data:
        target_user = user_data['user']
        
        # 자기 자신 제외
        if target_user.id == profile.id:
            continue
        
        # 대상 사용자도 동의 및 활성화 확인
        if not target_user.matching_consent or not target_user.service_active:
            continue
        
        # 대상 사용자의 이상형 프로필 확인
        try:
            target_ideal_type = target_user.ideal_type_profile
        except IdealTypeProfile.DoesNotExist:
            continue
        
        # 양방향 매칭 체크
        score1, criteria1 = check_match_criteria(ideal_type, target_user, ideal_type.match_threshold)
        score2, criteria2 = check_match_criteria(target_ideal_type, profile, target_ideal_type.match_threshold)
        
        # 양쪽 모두 임계값 이상이면 매칭 성공
        if score1 >= ideal_type.match_threshold and score2 >= target_ideal_type.match_threshold:
            matches.append({
                'target_user': target_user,
                'distance': user_data['distance'],
                'score': score1,
                'matched_criteria': criteria1,
                'mutual_score': score2,
            })
    
    # 점수 순으로 정렬
    matches.sort(key=lambda x: (x['score'], -x['distance']), reverse=True)
    
    return matches
```

**체크리스트**:
- [ ] `check_match_criteria` 함수 구현
- [ ] `find_matches` 함수 구현
- [ ] 양방향 매칭 로직 구현
- [ ] 필터링 로직 구현 (동의, 활성화)

---

### 4-3. 백엔드: 매칭 View 구현

**파일**: `backend/apps/matching/views.py` (새 파일 또는 기존 파일 수정)

**작업 내용**:
```python
from rest_framework.decorators import api_view, permission_classes
from rest_framework.permissions import IsAuthenticated
from rest_framework.response import Response
from rest_framework import status
from apps.users.models import User
from apps.matching.models import Match
from apps.matching.utils import find_matches
from django.utils import timezone

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_matchable_count(request):
    """
    매칭 가능 인원 수 조회
    """
    latitude = float(request.query_params.get('latitude'))
    longitude = float(request.query_params.get('longitude'))
    radius = float(request.query_params.get('radius', 0.05))  # 기본 50m
    
    try:
        profile = request.user.profile
        matches = find_matches(profile, latitude, longitude, radius)
        
        return Response({
            'success': True,
            'count': len(matches),
            'radius_km': radius
        }, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_400_BAD_REQUEST)

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def check_matches(request):
    """
    매칭 체크 (포그라운드)
    """
    try:
        profile = request.user.profile
        
        # 현재 위치 조회
        location = profile.location
        if not location:
            return Response({
                'success': False,
                'message': '위치 정보가 없습니다.'
            }, status=status.HTTP_400_BAD_REQUEST)
        
        # 매칭 찾기
        matches = find_matches(
            profile,
            float(location.latitude),
            float(location.longitude),
            radius_km=0.05
        )
        
        # 새 매칭이 있는지 확인 (중복 방지)
        new_matches = []
        for match_data in matches:
            target_user = match_data['target_user']
            
            # 이미 매칭된 사용자인지 확인
            existing_match = Match.objects.filter(
                user1=profile, user2=target_user
            ).first() or Match.objects.filter(
                user1=target_user, user2=profile
            ).first()
            
            if not existing_match:
                # 새 매칭 생성
                match = Match.objects.create(
                    user1=profile,
                    user2=target_user,
                    user1_latitude=location.latitude,
                    user1_longitude=location.longitude,
                    user2_latitude=target_user.location.latitude,
                    user2_longitude=target_user.location.longitude,
                    matched_criteria=match_data['matched_criteria']
                )
                
                # 알림 생성 (Phase 6에서 구현)
                # create_notification(profile, match)
                
                new_matches.append({
                    'match_id': match.id,
                    'target_user_id': target_user.id,
                    'matched_criteria': match_data['matched_criteria'],
                    'score': match_data['score']
                })
        
        return Response({
            'success': True,
            'has_new_match': len(new_matches) > 0,
            'new_matches': new_matches,
            'total_count': len(matches)
        }, status=status.HTTP_200_OK)
        
    except Exception as e:
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
```

**체크리스트**:
- [ ] `get_matchable_count` 함수 구현
- [ ] `check_matches` 함수 구현
- [ ] 중복 매칭 방지 로직
- [ ] Match 모델 생성 로직

---

### 4-4. 백엔드: 매칭 URL 추가

**파일**: `backend/apps/matching/urls.py`

**작업 내용**:
```python
from django.urls import path
from . import views

app_name = 'matching'

urlpatterns = [
    path('matchable-count/', views.get_matchable_count, name='matchable_count'),
    path('check/', views.check_matches, name='check_matches'),
]
```

**체크리스트**:
- [ ] URL 패턴 추가
- [ ] 경로 확인: `/api/matching/matchable-count/`, `/api/matching/check/`

---

### 4-5. 프론트엔드: API 클라이언트 함수 추가

**파일**: `Frontend/IdealMatchApp/src/services/api/apiClient.js`

**작업 내용**:
```javascript
/**
 * 매칭 가능 인원 수 조회
 * @param {number} latitude - 위도
 * @param {number} longitude - 경도
 * @param {number} radius - 반경 (km, 기본값: 0.05)
 * @returns {Promise<Object>} 인원 수
 */
async getMatchableCount(latitude, longitude, radius = 0.05) {
  try {
    const response = await this.request(
      `/matching/matchable-count/?latitude=${latitude}&longitude=${longitude}&radius=${radius}`,
      {
        method: 'GET',
      }
    );
    return {
      success: true,
      count: response.count,
    };
  } catch (error) {
    console.error('❌ 매칭 가능 인원 수 조회 실패:', error);
    return {
      success: false,
      error: error.message,
    };
  }
}

/**
 * 매칭 체크
 * @returns {Promise<Object>} 매칭 결과
 */
async checkMatches() {
  try {
    const response = await this.request('/matching/check/', {
      method: 'GET',
    });
    return {
      success: true,
      hasNewMatch: response.has_new_match,
      newMatches: response.new_matches || [],
    };
  } catch (error) {
    console.error('❌ 매칭 체크 실패:', error);
    return {
      success: false,
      error: error.message,
    };
  }
}
```

**체크리스트**:
- [ ] `getMatchableCount` 함수 추가
- [ ] `checkMatches` 함수 추가

---

### 4-6. 프론트엔드: 메인 화면 연동

**파일**: `Frontend/IdealMatchApp/src/screens/Main/MainScreen.js`

**작업 내용**:
- 매칭 가능 인원 수 표시
- 주기적 매칭 체크 (10초마다)
- 새 매칭 발견 시 알림 표시

**체크리스트**:
- [ ] 매칭 가능 인원 수 표시
- [ ] 주기적 매칭 체크 구현
- [ ] 새 매칭 발견 시 처리

---

### Phase 4 완료 기준

- [ ] 매칭 알고리즘이 정상 작동
- [ ] 매칭 가능 인원 수 조회 정상 작동
- [ ] 매칭 체크 정상 작동
- [ ] 프론트엔드에서 매칭 시스템 정상 작동

---

## Phase 5: 서비스 설정 기능 완성 ⭐⭐⭐

**목표**: 매칭 동의 ON/OFF 기능 완성

**예상 소요 시간**: 1시간

### 5-1. 백엔드: 동의 업데이트 View 구현

**파일**: `backend/apps/users/views.py`

**작업 내용**:
```python
@api_view(['POST'])
@permission_classes([IsAuthenticated])
def update_consent(request):
    """
    매칭 동의 및 서비스 활성화 업데이트
    """
    matching_consent = request.data.get('matching_consent')
    service_active = request.data.get('service_active')
    
    try:
        profile = request.user.profile
        
        if matching_consent is not None:
            profile.matching_consent = matching_consent
            profile.consent_updated_at = timezone.now()
        
        if service_active is not None:
            profile.service_active = service_active
        
        profile.save()
        
        return Response({
            'success': True,
            'message': '설정이 업데이트되었습니다.',
            'data': {
                'matching_consent': profile.matching_consent,
                'service_active': profile.service_active
            }
        }, status=status.HTTP_200_OK)
    except User.DoesNotExist:
        return Response({
            'success': False,
            'message': '프로필이 없습니다.'
        }, status=status.HTTP_404_NOT_FOUND)
```

**체크리스트**:
- [ ] `update_consent` 함수 구현
- [ ] URL 추가

---

### 5-2. 프론트엔드: API 클라이언트 함수 추가

**파일**: `Frontend/IdealMatchApp/src/services/api/apiClient.js`

**작업 내용**:
```javascript
/**
 * 매칭 동의 업데이트
 * @param {boolean} matchingConsent - 매칭 동의 여부
 * @param {boolean} serviceActive - 서비스 활성화 여부
 * @returns {Promise<Object>} 업데이트 결과
 */
async updateConsent(matchingConsent, serviceActive) {
  try {
    const response = await this.request('/users/consent/', {
      method: 'POST',
      body: JSON.stringify({
        matching_consent: matchingConsent,
        service_active: serviceActive,
      }),
    });
    return {
      success: true,
      data: response.data,
    };
  } catch (error) {
    console.error('❌ 동의 업데이트 실패:', error);
    throw error;
  }
}
```

**체크리스트**:
- [ ] `updateConsent` 함수 추가

---

### 5-3. 프론트엔드: 메인 화면에 동의 토글 추가

**파일**: `Frontend/IdealMatchApp/src/screens/Main/MainScreen.js`

**작업 내용**:
- 서비스 활성화/비활성화 토글 버튼 추가
- 동의 상태에 따라 위치 전송/매칭 체크 제어

**체크리스트**:
- [ ] 토글 버튼 추가
- [ ] 동의 상태에 따른 로직 구현

---

### Phase 5 완료 기준

- [ ] 매칭 동의 ON/OFF 기능 정상 작동
- [ ] 동의 상태에 따라 위치 전송/매칭 체크 제어

---

## Phase 6: 알림 시스템 완성 ⭐⭐⭐

**목표**: 알림 생성 및 조회 기능 완성

**예상 소요 시간**: 2-3시간

### 6-1. 백엔드: 알림 View 구현

**파일**: `backend/apps/matching/views.py`

**작업 내용**:
```python
from apps.matching.models import Notification

def create_notification(user, match):
    """
    매칭 시 알림 생성 (중복 방지)
    """
    # 5분 내 중복 알림 방지
    from datetime import timedelta
    recent_notification = Notification.objects.filter(
        user=user,
        match=match,
        boundary_entered_at__gte=timezone.now() - timedelta(minutes=5)
    ).first()
    
    if recent_notification:
        return recent_notification
    
    # 새 알림 생성
    notification = Notification.objects.create(
        user=user,
        match=match,
        boundary_entered_at=timezone.now(),
        is_active=True
    )
    
    return notification

@api_view(['GET'])
@permission_classes([IsAuthenticated])
def get_notifications(request):
    """
    알림 목록 조회
    """
    try:
        profile = request.user.profile
        notifications = Notification.objects.filter(
            user=profile
        ).select_related('match').order_by('-boundary_entered_at')
        
        notifications_data = []
        for notification in notifications:
            # 매칭된 상대방 정보
            other_user = notification.match.user1 if notification.match.user1 != profile else notification.match.user2
            
            notifications_data.append({
                'id': notification.id,
                'match_id': notification.match.id,
                'target_user': {
                    'id': other_user.id,
                    # 필요한 사용자 정보만 포함
                },
                'matched_criteria': notification.match.matched_criteria,
                'matched_at': notification.match.matched_at,
                'is_active': notification.is_active,
            })
        
        return Response({
            'success': True,
            'data': notifications_data
        }, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({
            'success': False,
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
```

**체크리스트**:
- [ ] `create_notification` 함수 구현 (중복 방지 포함)
- [ ] `get_notifications` 함수 구현
- [ ] 매칭 체크 시 알림 생성 로직 추가

---

### 6-2. 백엔드: 매칭 체크에 알림 생성 추가

**파일**: `backend/apps/matching/views.py`

**작업 내용**:
- `check_matches` 함수에서 새 매칭 발견 시 `create_notification` 호출

**체크리스트**:
- [ ] 알림 생성 로직 추가

---

### 6-3. 프론트엔드: API 클라이언트 함수 추가

**파일**: `Frontend/IdealMatchApp/src/services/api/apiClient.js`

**작업 내용**:
```javascript
/**
 * 알림 목록 조회
 * @returns {Promise<Object>} 알림 목록
 */
async getNotifications() {
  try {
    const response = await this.request('/notifications/', {
      method: 'GET',
    });
    return {
      success: true,
      data: response.data,
    };
  } catch (error) {
    console.error('❌ 알림 조회 실패:', error);
    return {
      success: false,
      error: error.message,
    };
  }
}
```

**체크리스트**:
- [ ] `getNotifications` 함수 추가

---

### 6-4. 프론트엔드: 알림 화면 구현

**파일**: `Frontend/IdealMatchApp/src/screens/Notification/` (새로 생성)

**작업 내용**:
- 알림 목록 화면 구현
- 매칭된 조건 정보 표시

**체크리스트**:
- [ ] 알림 목록 화면 구현
- [ ] 네비게이션 연결

---

### Phase 6 완료 기준

- [ ] 알림 생성 정상 작동
- [ ] 알림 목록 조회 정상 작동
- [ ] 중복 알림 방지 정상 작동
- [ ] 프론트엔드 알림 화면 정상 작동

---

## Phase 7: 통합 테스트 & 최종 점검 ⭐⭐⭐⭐⭐

**목표**: 전체 시스템 통합 테스트 및 버그 수정

**예상 소요 시간**: 2-4시간

### 7-1. 전체 플로우 테스트

**테스트 시나리오**:
1. 회원가입 → 로그인
2. 프로필 입력
3. 이상형 프로필 입력
4. 서비스 활성화
5. 위치 전송 확인
6. 매칭 체크 확인
7. 알림 확인

**체크리스트**:
- [ ] 전체 플로우 정상 작동
- [ ] 에러 케이스 처리 확인
- [ ] 에지 케이스 확인

---

### 7-2. 버그 수정

**작업 내용**:
- 발견된 버그 수정
- 에러 메시지 개선
- 로딩 상태 개선

---

### 7-3. 성능 최적화 (선택사항)

**작업 내용**:
- 쿼리 최적화
- API 응답 시간 확인
- 프론트엔드 렌더링 최적화

---

### Phase 7 완료 기준

- [ ] 전체 플로우 테스트 통과
- [ ] 주요 버그 수정 완료
- [ ] 사용자 경험 개선

---

## 📝 작업 시 주의사항

### 1. 순서 준수
- 각 Phase는 순서대로 진행 (의존성 있음)
- Phase 완료 후 다음 Phase로 진행

### 2. 테스트 중요
- 각 Phase 완료 후 반드시 테스트
- 백엔드 API는 Postman/curl로 먼저 테스트
- 프론트엔드는 실제 앱에서 테스트

### 3. 에러 처리
- 모든 API에 에러 처리 추가
- 사용자 친화적인 에러 메시지

### 4. 코드 품질
- 함수별 주석 추가
- 일관된 코드 스타일 유지

---

## 🎯 전체 예상 소요 시간

- **Phase 1**: 1-2시간
- **Phase 2**: 2-3시간
- **Phase 3**: 2-3시간
- **Phase 4**: 4-6시간 (가장 복잡)
- **Phase 5**: 1시간
- **Phase 6**: 2-3시간
- **Phase 7**: 2-4시간

**총 예상 시간**: 14-22시간

---

**마지막 업데이트**: 2025-01-15
