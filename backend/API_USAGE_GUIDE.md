...
**기능:**
- 아이디와 전화번호를 받아 사용자 확인
- 일치하면 인증번호를 전화번호로 발송
- 비밀번호 재설정 프로세스 시작

**필요한 때:**
- ✅ 비밀번호 찾기 화면에서 아이디와 전화번호를 입력하고 "인증번호 발송" 버튼을 누를 때
- ✅ 사용자가 비밀번호를 잊어버렸을 때


**예시 시나리오:**
```
사용자: "비밀번호 찾기 화면에서 아이디: user123, 전화번호: 010-1234-5678 입력 → 인증번호 발송 버튼 클릭"
앱: API 16 호출 → 성공 시 "인증번호를 발송했습니다" 메시지 표시 → 인증번호 입력 화면으로 이동
```
...
- 모든 조건(성별, 키, 나이, MBTI, 성격, 관심사)은 최소 1개 이상 필수

**주요 필드:**
- `preferred_gender`: 선호 성별 리스트 (배열, 최소 1개 이상 필수)
  - `["M"]`: 남성만 매칭
  - `["F"]`: 여성만 매칭
  - `["M", "F"]`: 남성과 여성 모두 매칭 가능
- `height_min`, `height_max`: 키 범위 (cm)
- `age_min`, `age_max`: 나이 범위
- `preferred_mbti`: 선호 MBTI 리스트 (배열, 최소 1개 이상 필수)
- `preferred_personality`: 선호 성격 리스트 (배열, 최소 1개 이상 필수)
- `preferred_interests`: 선호 관심사 리스트 (배열, 최소 1개 이상 필수)
...
9. **API 16-18 (비밀번호 재설정)**: 
   - API 16: 아이디와 전화번호 일치 확인 (보안상 상세 정보 노출하지 않음)
